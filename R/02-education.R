...
parent_edu_all <- parent_edu_all %>%
  mutate(across(
    ...
    ~ case_when(
      ...
      .x == -91 ~ -1,
      .x %in% c(-98) ~ -3,
      .x %in% c(-999, -99, -94) ~ -2, # REVIEW: Removed 19, which would get remapped to 'script error' otherwise (“Qualification, level unspecified”)
      TRUE ~ .x
    )
  ))
...
parent_edu_all <- parent_edu_all %>%
  mutate(
    ...
    # REVIEW: changed from S1 -> S4 (was reversed for mother's edu)
    educdtlma = case_when(
